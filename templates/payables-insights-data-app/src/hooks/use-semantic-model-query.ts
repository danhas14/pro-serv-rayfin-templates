...
export function useSemanticModelQuery(
    ...
    // Superseded queries are ignored so a fast slider or window change cannot
    // land an older result over a newer one. The loading flag is set inside
    // `run`, which the lint rule cannot see past.
    useEffect(() => {
        let cancelled = false;
        // eslint-disable-next-line react-hooks/set-state-in-effect -- fetching from the Fabric SDK is the external system this effect synchronises with
        void run(() => cancelled);
        return () => {
            cancelled = true;
        };
