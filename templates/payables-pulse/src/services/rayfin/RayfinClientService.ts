...
import type { PayablesSchema } from '../../../rayfin/data/schema';

import { createOfflineClient } from './OfflineDemoClient';

/** Dev-only harness used to drive the UI without a Fabric backend. */
export const OFFLINE_DEMO =
  import.meta.env.DEV && import.meta.env.VITE_OFFLINE_DEMO === '1';
...
export class RayfinClientService {
  ...
  private _client: RayfinClient<PayablesSchema> | null = null;
  ...
  public initialize(
  ...
  ): RayfinClient<PayablesSchema> {
    if (this._client) return this._client;

    if (OFFLINE_DEMO) {
      console.log('🧪 Offline demo harness enabled — no backend calls');
      this._client = createOfflineClient() as unknown as RayfinClient<PayablesSchema>;
      return this._client;
    }
    ...
    this._client = new RayfinClient<PayablesSchema>({
      baseUrl: baseUrl,
   ...
   */
  public getClient(): RayfinClient<PayablesSchema> {
    if (!this._client) {
 ...
 * @throws Error if the client is not initialized
 */
export function getRayfinClient(): RayfinClient<PayablesSchema> {
